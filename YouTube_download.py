...
def download_youtube_video(url, output_directory):
    ...
    try:
        ...
        if os.path.exists(video_filepath):
            ...
            increment = 1
            while os.path.exists(f"{output_directory}/{video_title}_{increment}.mp4"):
                increment += 1
...
def download_videos_from_file(file_path, output_directory):
    """
    Download YouTube videos from a file containing a list of URLs.

    Args:
        file_path (str): Path to the text file containing YouTube video URLs.
        output_directory (str): The directory to save the downloaded videos.
...
if __name__ == "__main__":
    ...
    print("1. Single video (Enter 'S')")
    print("2. Video URLs from a file (Enter 'F')")
    ...
    if choice == 'S':
        ...
        print("Enter the path to the text file containing YouTube video URLs:")
        file_path = sys.stdin.readline().strip()
        
        # Specify the output directory using os.getenv('USER')
        output_directory = f"/home/{os.getenv('USER')}/Videos/YouTube_downloads"
        
        # Download videos from the file
